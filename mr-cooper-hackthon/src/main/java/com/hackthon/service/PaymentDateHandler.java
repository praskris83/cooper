...
public class PaymentDateHandler {
  ...
  private ConversationContext readyToChangeHandler(String contextKey, ConversationContext context,
    ...
    if (context.getConversationStatus() == ConversationStatus.INIT
        && findReadyToChangeStringPattern(msg)) {
  ...
  public void confirmChange(ConversationContext context, Conversation conversation) {
    boolean isAccepted = CommonUtils.isAccept(conversation.getMessage());
    if (isAccepted) {
      conversation.setEndStatus(ConversationStatus.CHANGE_COMPLETED);
    } else {
      context.setNegativeScore(context.getNegativeScore() > 0 ? context.getNegativeScore() + 1 : 1);
    }
  }
